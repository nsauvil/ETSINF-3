...
import re
import numpy as np
#   Participantes: KlÃ©ber Zapata Zambrano, Rafael EstellÃ©s Tatay, Noelia Saugar Villar, Mario DÃ­az Acosta, Naiara PÃ©rez Leizaola.
...
class SAR_Project:
    ...
    Se pueden aÃƒÂ±adir nuevas variables y nuevos metodos
    Los metodos que se aÃƒÂ±adan se deberan documentar en el codigo y explicar en la memoria
    """
    ...
    def __init__(self):
        ...
        Puedes aÃƒÂ±adir mÃƒÂ¡s variables si las necesitas 

        """
        #posting list= lista de documetnos(meterlso como nÃºmeros) que contiene una palabra
        self.index = {'title': {},
                      'date': {},
                      'keywords': {},
                      'article': {},
        ...
        #Cojo la palabra le hago stema (casas, cas) y busco con un or en el Ã­ndice invertido
        self.sindex = {'title': {},
                      'date': {},
                      'keywords': {},
                      'article': {},
        ...
        self.news = {} # hash de noticias --> clave entero (newid), valor: la info necesaria para diferenciar la noticia dentro de su fichero (doc_id y posiciÃƒÂ³n dentro del documento)
        ...
        #################
           ...
           # newid = len(self.news) #conseguimos el id posterior al de la ultima noticia agregada
            ...
            for new in jlist: #recorremos la lista de noticias que hay en el fichero
                ...
                if self.multifield:  #ExtensiÃ³n multifield
                ...
                for f in multifield:  
                    ...
                    for t in tokens: #recorremos los tokens
                        ...
                        if not self.index[f].get(t,0): #si el token no estÃ¡ en el Ã­ndice  
                            if not self.positional:
                                self.index[f][t] = {self.newid: 1}
                        #Implementar para el positional 
                            else: self.index[f][t] = {self.newid: [pos_token]}
                        else:  
                            if not self.index[f][t].get(self.newid,0): #aÃ±adir la noticia si no estÃ¡ en el token 
                                if not self.positional:
                                    self.index[f][t][self.newid] = 1
    ...
    def make_stemming(self):
        ...
        ####################################################
        # Si se activa la funciÃ³n multifield
        if self.multifield:
            multifield = ['title', 'date', 'keywords', 'article', 'summary']
        else:
            multifield = ['article']
        for field in multifield:
            # Se aplica stemming a cada token del self.index[field] y se aÃ±ade al indice de stems
            # En este caso solo se guarda la noticia, no la posiciÃ³n
    ...
    def make_permuterm(self):
        ...
        for field in multifield:
            for token in self.index[field].keys():
                #Lista donde almacenar la permutaciÃ³n de cada token
                permutation = []
                #AÃ±adimos el $ al final para hacer permuterm
                tokenA = token + '$'
                ...
                for p in permutation:
                    ...
                    if p not in self.ptindex:
                        #aÃ±adimos el primer token a esa permutaciÃ³n
                        ...
                        if token not in self.ptindex[field][p]:
                            #AÃ±adimos el token a esa permutaciÃ³n
                            self.ptindex[field][p] += token
    ...
    def show_stats(self):
       ...
       # pass
        ...
        print("========================================")
        print("Number of indexed days: " + str(len(self.docs))) #La longitud del hash de documentos es el nÃºmero de documentos indexados
        print("----------------------------------------")
        print("Number of indexed news: " + str(len(self.news))) #La longitud del hash de noticias es el nÃºmero de noticias indexadas
        print("----------------------------------------")
        print("TOKENS:")
        ...
        if self.multifield:
            ...
            print("\t# of tokens in 'article': " + str(len(self.index['article']))) #Se busca en 'article' porque por ahora sÃ³lo se indexa el contenido del campo 'article' del diccionario de la noticia.
        print("----------------------------------------")
        if self.permuterm:
            print("PERMUTERMS:")
            if self.multifield:
    ...
    def solve_query(self, query, prev={}):
        ...
        #arreglamos los parÃ©ntesis para que se queden aislados en el split 
        query = query.replace('"', '')
        query = query.replace('(', ' ( ')
        query = query.replace(')', ' ) ')
        query = query.replace(':', ' : ')
        ...
        i=0
        if ':' in q: #MULTIFIELD
            nm = q.count(':') #contar cuÃ¡ntos fields hay
            while nm > 0: #analizar todos los campos
                pos = q.index(':') #posiciÃ³n de : en la query
    ...
    def get_posting(self, term, field='article', wildcard='False'):
       ...
       #FALTA EL DE POSICIONALES 
            ...
            posting = self.index[field].get(term, []) #implementaciÃ³n para la entrega obligatoria 
            lista = [] 
            if posting ==[]: return []  
        ...
        if terms[0] in self.index[field]: #si el primer termino se encuentra indexado 
            for posting_list in self.index[field][terms[0]].items(): #recorre las posting_list de noticias del primer termino
                ...
                for posicion in lista_posiciones: #recorre las posiciones del termino en la noticia actual 
                    i=1
                    corrido = True
                    while i<len(terms) and corrido: #recorremos el resto de terminos comprobando que estÃ¡n de seguido 
                        ...
                        if self.index[field].get(terms[i],0): #comprueba si el termino actual se encuentra indexado 
                            ...
                            corrido = False
                        i+=1
                    if corrido: #si han estado todos los terminos de corrido se aÃ±ade la noticia a la solucion 
                       # print(noticia)
                        res += [noticia]
    ...
    def reverse_posting(self, p):
        ...
        for i in p:
            ...
            news.remove(i)
        #Devolvemos las noticias sin las que tenÃ­a p
        return news
    ...
    def or_posting(self, p1, p2):
        ...
        """
        #aÃ±adimos todos los terminos a la lista
        res = []
        i = 0
        j = 0
    ...
    def not_or_posting(self, p1, p2):
        ...
        #while i < len(p1) and j < len(p2):
                ...
                #j += 1
                #news.remove(p1[i])
        #Terminos que no han sido aÃ±adidos aun al resultado
        #for k in range(i, len(p1)):
            #news.remove(p1[i])
    ...
    def solve_and_show(self, query):
        ...
        for noticia in result:
            ...
            pos = n['posicion']  #posiciÃ³n de la noticia dentro del documento
            ...
            #comprobar si la funciÃ³n snippets estÃ¡ activada
            if self.show_snippet:
                print('#{}'.format(i))  #orden recuperaciÃ³n noticia
               ...
               #si no se activa el snippet, mostrar una lÃ­nea por noticia 
                print('#{} \t ({}) ({}) ({}) {} ({}) '.format(i,score,noticia,n['date'], n['title'], n['keywords']))
            
            i = i+1
            if not self.show_all and i > self.SHOW_MAX:
               #si no estÃ¡ activada la funciÃ³n show_all y ya se han mostrado todas las noticias permitidas(max), detener el bucle 
                break
    ...
    def jaccard(self, doc, query):
        """
        jaccard(A,B) =|A intersecciÃ³n B| / |A uniÃ³n B| 
        param:  "doc": noticia
        ...
        #con MULTIFIELD se analizan: 'title, article, summary, keywords y date' 
         #en el Indexer bÃ¡sico solo se analiza el campo 'article' 
        d = doc['article']
    ...
    def snippet(self,n,query):
       ...
       '''
        ...
        #Quitar todos los sÃ­mbolos no necesarios en la query 
        query = query.replace('"', '') 
        query = query.replace('?', '')
        query = query.replace('*', '')
        ...
        for p in query:  #recorrer las palabras de la query 
            ...
            if p in local:
               ...
               # Mostrar (si se puede) la frase donde se encuentra la palabra buscada
                ...
                if pos_i < 0:  #revisar que la palabra no estÃ¡ al comienzo de la noticia 
                    pos_i = 0
                pos_d = pos + 6 #a la derecha de la palabra 
                if pos_d > len(local) - 1: #revisar que la palabra no estÃ¡ al final de la noticia
                    pos_d = len(local) - 1

               #Comprobar que no estÃ¡ al principio 
                if pos_i > 0:
                    snippet_aux = snippet_aux + '...'  #los puntos indican que no es la frase completa 

                snippet_aux += " ".join(local[pos_i:pos_d + 1])

                #Comprobar que no estÃ¡ al final
                if pos_d < len(local) - 1:
                    snippet_aux += '...'

                cont += 1
                #Comprobar que no hemos revisado todas las palabras de query aÃºn
    ...
    def rank_result(self, result, query):
        ...
        for n in result:
            ...
            pos = ntc['posicion']  #posiciÃ³n de la noticia dentro del documento
            fh = open(self.docs[idDoc])
